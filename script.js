...
const group = category => ['10m_range','25m_range','50m_range'].includes(category) ? 'range' : ['events','competitions'].includes(category) ? 'event' : category;
...
async function loadContent() {
  ...
  if (!gallery.error && gallery.data?.length) { $('#gallery-grid').innerHTML = gallery.data.map((g,i) => `<button class="gallery-item ${i===0?'tall':i===2?'wide':''}" data-category="${group(g.category)}" data-image="${esc(g.image_url)}" style="background-image:url('${esc(g.image_url)}')"><span>${esc(g.title)}</span></button>`).join(''); wireGallery(); }
